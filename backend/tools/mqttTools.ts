...
const mqtt_publish = tool({
  ...
  execute: async (params, options) => {
    const { updateStatus, userId, mqttService } = getContext(options);
    updateStatus(`Publishing to ${params.topic}...`);
    try {
      if (!mqttService) {
        return JSON.stringify({ error: "MQTT service not available" });
      }
      const qos = (params.qos ?? 0) as 0 | 1 | 2;
      await mqttService.publish(userId, params.topic, params.payload, qos, params.retain ?? false);
      return JSON.stringify({ success: true, message: `Published to ${params.topic}` });
    } catch (err) {
      return JSON.stringify({ error: err instanceof Error ? err.message : "Failed to publish" });
    }
  },
});
...
const mqtt_subscribe = tool({
  ...
  execute: async (params, options) => {
    const { updateStatus, userId, agentId, conversationId, mqttRepository, mqttService } = getContext(options);
    updateStatus(`Subscribing to ${params.topic}...`);
    try {
      if (!mqttRepository || !mqttService) {
        return JSON.stringify({ error: "MQTT service not available" });
      }
      ...
      return JSON.stringify({ error: err instanceof Error ? err.message : "Failed to subscribe" });
    }
  },
});
...
const mqtt_unsubscribe = tool({
  ...
  execute: async (params, options) => {
    const { updateStatus, agentId, userId, mqttRepository, mqttService } = getContext(options);
    ...
    try {
      ...
      return JSON.stringify({ success: true, message: `Unsubscribed from ${params.topic}` });
    } catch (err) {
      return JSON.stringify({ error: err instanceof Error ? err.message : "Failed to unsubscribe" });
    }
  },
});
...
const mqtt_list_subscriptions = tool({
  ...
  execute: async (_params, options) => {
    const { updateStatus, agentId, mqttRepository } = getContext(options);
    ...
    try {
      ...
      return JSON.stringify({ error: err instanceof Error ? err.message : "Failed to list subscriptions" });
    }
  },
});
...
const mqtt_get_recent = tool({
  ...
  execute: async (params, options) => {
    const { updateStatus, userId, mqttRepository } = getContext(options);
    ...
    try {
      ...
      return JSON.stringify({ error: err instanceof Error ? err.message : "Failed to get messages" });
    }
  },
});

/** MQTT tools - include when agent has "mqtt" built-in tool enabled */
export const mqttTools: Record<string, AiTool> = {
  mqtt_publish,
  mqtt_subscribe,
  mqtt_unsubscribe,
  mqtt_list_subscriptions,
