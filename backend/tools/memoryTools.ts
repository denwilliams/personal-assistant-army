...
const remember = tool({
  ...
  execute: async (params, options) => {
    const { updateStatus, agentId, memoryRepository, generateEmbedding } = getContext(options);
...
const forget = tool({
  ...
  execute: async (params, options) => {
    const { updateStatus, agentId, memoryRepository } = getContext(options);
...
/** Memory tools - include when agent has "memory" built-in tool enabled */
export const memoryTools: Record<string, AiTool> = {
  remember,
  recall,
  forget,
  promote_memory,
